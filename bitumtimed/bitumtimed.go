// Copyright (c) 2017 The Bitum developers
...
import (
	...
	"github.com/bitum-project/bitumtime/api/v1"
...
// BitumtimeStore application context.
type BitumtimeStore struct {
...
func (d *BitumtimeStore) sendToBackend(w http.ResponseWriter, method, route, contentType, remoteAddr string, body *bytes.Reader) {
...
func (d *BitumtimeStore) proxyStatus(w http.ResponseWriter, r *http.Request) {
...
func (d *BitumtimeStore) proxyTimestamp(w http.ResponseWriter, r *http.Request) {
	b, err := ioutil.ReadAll(r.Body)
	r.Body.Close()
	if err != nil {
		util.RespondWithError(w, http.StatusBadRequest,
...
func (d *BitumtimeStore) proxyVerify(w http.ResponseWriter, r *http.Request) {
	b, err := ioutil.ReadAll(r.Body)
	r.Body.Close()
	if err != nil {
		util.RespondWithError(w, http.StatusBadRequest,
...
func (d *BitumtimeStore) status(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
...
func (d *BitumtimeStore) timestamp(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
...
func (d *BitumtimeStore) verify(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
...
func _main() error {
	...
	if !fileExists(loadedCfg.HTTPSKey) &&
		...
		err := util.GenCertPair("bitumtimed", loadedCfg.HTTPSCert,
			loadedCfg.HTTPSKey)
		if err != nil {
